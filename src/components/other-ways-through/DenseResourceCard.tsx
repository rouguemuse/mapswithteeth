  ...
  return (
    <>
      <article
        ...
        className="bg-[#EEE8DD] border-2 border-[#1C1D1D] rounded-md p-3 sm:p-3.5 hover:border-[#971F26] hover:shadow-sm transition-all duration-150 cursor-pointer group select-none relative shadow-2xs"
      >
        {/* Top Row: Icon + Title/Subtitle + Badges */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[#D9D1C4] pb-2 mb-2">
          <div className="flex items-center gap-2.5 min-w-0">
            {/* Left Icon Pill */}
            <div className="w-7 h-7 rounded-full bg-[#1C1D1D] group-hover:bg-[#971F26] transition-colors flex items-center justify-center shrink-0 shadow-2xs">
              {getIcon()}
            </div>

            <div className="min-w-0">
              <div className="flex items-center gap-1.5 flex-wrap">
                <h3 className="font-serif font-bold text-sm sm:text-base text-[#1C1D1D] group-hover:text-[#971F26] transition-colors leading-tight">
                  {resource.name}
                </h3>
              </div>
              <p className="text-[10px] font-mono text-stone-600 font-semibold truncate">
          ...
          <div className="flex flex-wrap items-center gap-1 shrink-0 self-start sm:self-auto font-mono text-[9px]">
            {isLikelyMatch && (
              <span className="px-1.5 py-0.5 bg-[#E8F3EB] border border-[#2D5A3D] text-[#2D5A3D] font-bold rounded-xs flex items-center gap-1">
            ...
            {isWorthChecking && (
              <span className="px-1.5 py-0.5 bg-[#FEF3C7] border border-[#D97706] text-[#92400E] font-bold rounded-xs flex items-center gap-1">
            ...
            {isStatutory ? (
              <span className="px-1.5 py-0.5 bg-[#1C1D1D] text-white font-bold rounded-xs tracking-wider">
                STATUTORY RIGHT
              </span>
            ) : (
              <span className="px-1.5 py-0.5 bg-[#F5F1E8] border border-[#D9D1C4] text-stone-700 font-semibold rounded-xs">
                {resource.scope === "NATIONWIDE" ? "NATIONWIDE" : "NONTRADITIONAL ROUTE"}
              </span>
            )}

            {resource.referralRequirement?.toLowerCase().includes("advocate") ? (
              <span className="px-1.5 py-0.5 bg-[#FEF3C7] border border-[#F59E0B] text-[#78350F] font-semibold rounded-xs">
                WARM REFERRAL
              </span>
            ) : (
              <span className="px-1.5 py-0.5 bg-[#E2E8F0] border border-[#94A3B8] text-[#334155] font-semibold rounded-xs">
                LATERAL BYPASS
              </span>
            )}

            <span className="px-1.5 py-0.5 bg-[#E8F3EB] border border-[#2D5A3D] text-[#2D5A3D] font-bold rounded-xs">
              ✓ VERIFIED
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-2 text-xs">
          ...
          <div className="p-2 bg-[#F5F1E8] border border-[#D9D1C4] rounded-xs space-y-0.5">
            <span className="text-[8.5px] font-mono uppercase tracking-wider text-[#971F26] font-bold block">
              {whySurfacedFact ? "Why This Surfaced:" : "Why It's Missed:"}
            </span>
            <p className="text-stone-900 font-medium text-[10.5px] leading-tight line-clamp-3">
              {whyMissedText}
            </p>
          </div>
          ...
          <div className="p-2 bg-[#F5F1E8] border border-[#D9D1C4] rounded-xs space-y-0.5">
            <span className="text-[8.5px] font-mono uppercase tracking-wider text-[#1C1D1D] font-bold block">
              What It Can Do:
            </span>
            <p className="text-stone-900 font-medium text-[10.5px] leading-tight line-clamp-3">
              {whatItCanDoText}
          ...
          <div className="p-2 bg-[#F5F1E8] border border-[#D9D1C4] rounded-xs space-y-0.5">
            ...
            <p className="text-stone-800 text-[10.5px] leading-tight line-clamp-3 font-mono">
              {theCatchText}
            </p>
          </div>

          {/* Column 4: VERIFICATION & SOURCE */}
          <div className="p-2 bg-[#F5F1E8] border border-[#D9D1C4] rounded-xs space-y-0.5 flex flex-col justify-between">
            <div>
              <span className="text-[8.5px] font-mono uppercase tracking-wider text-[#2D5A3D] font-bold block">
                Verification & Source:
              </span>
              <p className="text-[10px] font-mono text-stone-700 leading-tight">
                ✓ {verificationText}
              </p>
              <p className="text-[9.5px] text-stone-600 truncate mt-0.5">
                {resource.primaryAuthoritativeSource}
              </p>
            </div>

            <div className="flex items-center justify-between pt-1 border-t border-[#E5DEC9] mt-1 text-[9.5px] font-mono text-[#971F26] font-bold">
        ...
        {evidenceRequired && evidenceRequired.length > 0 && (
          <div className="mt-2 pt-1.5 border-t border-[#D9D1C4] flex items-start gap-1.5 text-[10px] text-stone-800 bg-[#F5F1E8] p-1.5 rounded-xs border border-[#E5DEC9]">
            <FileText className="w-3 h-3 text-stone-600 shrink-0 mt-0.5" />
            <div className="leading-tight">
              <strong className="text-[#1C1D1D] font-mono text-[9px] uppercase tracking-wider block">
