    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-14 select-none font-sans">
      ...
      <div className="border-b border-[#D9D1C4] pb-6 space-y-2">
        ...
        <p className="text-base sm:text-[17px] text-stone-900 leading-relaxed font-sans font-medium">
          Maps With Teeth is an independent public-interest initiative in development. Contributions help fund primary-source resource verification, field research across Texas counties, digital infrastructure, and the development of tools designed to make fragmented systems easier to navigate.
        </p>
      </div>
      ...
      <section className="space-y-4 bg-[#EEE8DD] border-2 border-[#1C1D1D] rounded-xl p-6 sm:p-8 shadow-xs">
        <div className="flex items-center gap-2 text-[#1C1D1D]">
          ...
          <h2 className="text-xl sm:text-2xl font-serif font-bold">
            Organizational Structure & Fiduciary Disclosures
          </h2>
        </div>

        <div className="p-4 bg-[#F5F1E8] border border-[#D9D1C4] rounded-lg text-xs sm:text-sm text-stone-800 font-mono space-y-2 shadow-2xs">
          <div className="flex items-center gap-2 font-bold text-[#971F26] uppercase text-xs">
            <Info className="w-4 h-4" />
            <span>Institutional & Tax Disclosure</span>
          </div>
          <p className="font-sans leading-relaxed text-stone-900">
          ...
          <p className="text-xs font-sans text-stone-700 italic pt-1.5 border-t border-[#D9D1C4]">
      ...
      {/* Central Texas 6-Month Pilot Requirement Model */}
        ...
        <div className="border-b border-[#D9D1C4] pb-3 flex flex-wrap items-center justify-between gap-2">
          <div>
            <span className="text-xs font-mono uppercase tracking-widest text-[#971F26] font-bold block">
              RESEARCH REQUIREMENTS & BUDGET SPECIFICATION
            </span>
            <h2 className="text-xl sm:text-2xl font-serif font-bold text-[#1C1D1D]">
              What a Fully Funded Six-Month Central Texas Pilot Requires (${PILOT_CONFIG.totalTarget.toLocaleString()})
            </h2>
          </div>
          <div className="px-3 py-1.5 bg-[#EEE8DD] border border-[#1C1D1D] rounded text-xs font-mono text-[#1C1D1D] font-bold shadow-2xs">
            Cohort Capacity: ~{PILOT_CONFIG.cohortSize} Participants
          </div>
        </div>

        <p className="text-sm text-stone-800 leading-relaxed font-sans">
          Operating across Travis, Williamson, Hays, and Bastrop counties, the six-month pilot pairs field-tested Bridge Navigation with our Survivor Gap Fund to demonstrate barrier-removal cost efficiency compared to fragmented legacy systems.
        </p>
            ...
            {PILOT_CONFIG.categories.map((cat) => (
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <span className="font-bold text-sm sm:text-base text-[#1C1D1D] font-serif">
                    {cat.name}
                  </span>
      ...
      <section className="space-y-4">
        <div className="border-b border-[#D9D1C4] pb-3">
          ...
          <p className="text-xs sm:text-sm text-stone-700 mt-1 font-sans">
            For institutional foundations, donor-advised funds (DAFs), and corporate matching contributions. Inquiries route directly to <strong>hello@mapswithteeth.org</strong>.
          </p>
        </div>
        <SupportInquiryForm />
      </section>
